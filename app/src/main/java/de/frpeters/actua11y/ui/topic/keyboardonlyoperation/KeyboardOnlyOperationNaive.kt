...
/**
 * Naive implementation. Same toolbar as [KeyboardOnlyOperationBetter], same layout, same
 * behaviour for a sighted touch user — the only difference is accessibility semantics.
 */
@Composable
fun KeyboardOnlyOperationNaive(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
            ...
            // NAIVE: built with a bare pointerInput gesture detector instead of
            // Modifier.clickable. This still responds to a touch tap, but carries no semantics
            // of any kind — no click action, not focusable — so it is unreachable by keyboard
            // Tab navigation, switch access, or a TalkBack swipe, all at once.
            Text(
                text = bookmarkLabel,
                modifier = Modifier
                    .pointerInput(Unit) {
                        detectTapGestures { lastAction = bookmarkLabel }
                    }
                    .padding(8.dp)
        ...
        DeveloperNote(body = stringResource(R.string.keyboard_only_operation_developer_note_naive))
    }
}
...
@Composable
private fun KeyboardOnlyOperationNaivePreview() {
    MaterialTheme {
        KeyboardOnlyOperationNaive()
    }
}
