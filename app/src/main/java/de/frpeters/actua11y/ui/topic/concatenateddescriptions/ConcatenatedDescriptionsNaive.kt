...
/**
 * Naive implementation. Same order card, same visible lines, same single TalkBack stop as
 * [ConcatenatedDescriptionsBetter]. The card was deliberately given one combined description —
 * the accessibility work was started, just not finished.
 */
@Composable
fun ConcatenatedDescriptionsNaive(modifier: Modifier = Modifier) {
    ...
    // NAIVE: joined with plain spaces. Every segment is present and in a sensible order, but
    // nothing marks where one ends and the next begins, so the listener hears one continuous
    // stream at speech tempo and has to work out the boundaries themselves.
    ...
    Column(
        ...
        DeveloperNote(body = stringResource(R.string.concatenated_descriptions_developer_note_naive))
    }
}
...
@Composable
private fun ConcatenatedDescriptionsNaivePreview() {
    MaterialTheme {
        ConcatenatedDescriptionsNaive()
    }
}
