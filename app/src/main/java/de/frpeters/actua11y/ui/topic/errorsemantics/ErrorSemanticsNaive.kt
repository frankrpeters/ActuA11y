...
/**
 * Naive implementation. Same email field as [ErrorSemanticsBetter], same layout, same behaviour
 * for a sighted touch user — the only difference is accessibility semantics.
 */
@Composable
fun ErrorSemanticsNaive(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        // NAIVE: isError = true only changes the field's colours. Nothing here tells an
        // accessibility service the field is in error — that is a separate, ordinary Text below,
        // with no semantic link back to the field at all.
        OutlinedTextField(
            value = email,
        ...
        Text(
            text = stringResource(R.string.error_semantics_error_message),
        ...
        DeveloperNote(body = stringResource(R.string.error_semantics_developer_note_naive))
    }
}
...
@Composable
private fun ErrorSemanticsNaivePreview() {
    MaterialTheme {
        ErrorSemanticsNaive()
    }
}
