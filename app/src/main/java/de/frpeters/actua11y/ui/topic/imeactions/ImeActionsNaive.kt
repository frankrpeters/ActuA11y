...
/**
 * Naive implementation. Same two-field form as [ImeActionsBetter], same layout, same behaviour
 * for a sighted touch user — the only difference is accessibility semantics.
 */
@Composable
fun ImeActionsNaive(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        // NAIVE: default KeyboardOptions on both fields — the keyboard's own action button is
        // left at its generic default, which does not advance to the next field or submit
        // anything. No KeyboardActions handler is registered either.
        OutlinedTextField(
            value = name,
        ...
        DeveloperNote(body = stringResource(R.string.ime_actions_developer_note_naive))
    }
}
...
@Composable
private fun ImeActionsNaivePreview() {
    MaterialTheme {
        ImeActionsNaive()
    }
}
