...
/**
 * Naive implementation. Same sign-up form as [AutofillHintsBetter], same layout, same behaviour
 * for a sighted touch user — the only difference is accessibility semantics.
 */
@Composable
fun AutofillHintsNaive(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        // NAIVE: no ContentType declared on either field. A password manager has no declared hint
        // to detect or fill either field with, silently forcing manual entry — disproportionately
        // costly for a TalkBack user compared to a sighted one.
        OutlinedTextField(
            value = username,
        ...
        DeveloperNote(body = stringResource(R.string.autofill_hints_developer_note_naive))
    }
}
...
@Composable
private fun AutofillHintsNaivePreview() {
    MaterialTheme {
        AutofillHintsNaive()
    }
}
