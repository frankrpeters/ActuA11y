...
/**
 * Better implementation. Same two-step checkout as [RedundantEntryNaive], same fields, same
 * buttons — the only difference is that step 2 starts from what step 1 already collected.
 */
@Composable
fun RedundantEntryBetter(modifier: Modifier = Modifier) {
    ...
    Column(
        ...
        if (step == 1) {
            ...
            Button(
                    onClick = {
                        // BETTER: the billing fields are pre-filled from the delivery address the
                        // user already entered, rather than asking for it again (WCAG 2.2 SC
                        // 3.3.7). Only while they are still untouched — once the user has typed
                        // a different billing address, going back and forth must not overwrite it.
                        if (billing == AddressFormState()) billing = delivery
                        step = 2
                    },
        ...
        DeveloperNote(body = stringResource(R.string.redundant_entry_developer_note_better))
    }
}
...
@Composable
private fun RedundantEntryBetterPreview() {
    MaterialTheme {
        RedundantEntryBetter()
    }
}
